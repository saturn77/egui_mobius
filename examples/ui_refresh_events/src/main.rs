...
use egui_mobius::types::{Edge, Value};
use log::{info, warn};
...
impl UiApp {
    fn new(
        ...
        Self {
            logger_text: Value::new(
                "**** Welcome to egui_mobius ui_refresh_events example ....\n\n".to_string(),
            ),
            signal_to_backend,
            slot_on_uiapp,
            update_needed,
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        self.slot_on_uiapp.start({
            ...
            move |event: ProcessedType| {
                let mut logger: egui_mobius::types::ValueGuard<'_, String> =
                    logger_text_clone.lock().unwrap();
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                if ui.button("Foo").clicked() {
                    self.signal_to_backend
                        .send(EventType::Foo {
                            id: 1,
                            message: "Foo Button Clicked".to_string(),
                        })
                        .unwrap();
                }
                if ui.button("Bar").clicked() {
                    self.signal_to_backend
                        .send(EventType::Bar {
                            id: 2,
                            message: "Bar Button Clicked".to_string(),
                        })
                        .unwrap();
                }
            });
            ...
            ui.horizontal(|ui| {
                if ui.button("Clear Logger").clicked() {
                    self.signal_to_backend
                        .send(EventType::ApplicationCommand("Clear Logger".to_string()))
                        .unwrap();
                }
                if ui.button("OS Info").clicked() {
                    self.signal_to_backend
                        .send(EventType::ApplicationCommand("OS Info".to_string()))
                        .unwrap();
                }
                if ui.button("Version Info").clicked() {
                    self.signal_to_backend
                        .send(EventType::ApplicationCommand("Version Info".to_string()))
                        .unwrap();
                }
                if ui.button("Shutdown").clicked() {
                    self.signal_to_backend
                        .send(EventType::ApplicationCommand("Shutdown".to_string()))
                        .unwrap();
                }
            });
        ...
        move |event| {
            ...
            match &event {
                EventType::Foo { id, message } => {
                    ...
                    let processed_msg = ProcessedType::Foo {
                        _id: *id,
                ...
                EventType::Bar { id, message } => {
                    ...
                    let processed_msg = ProcessedType::Bar {
                        _id: *id,
                        message: log_msg.clone(),
                    };
                    process_event!(queue, logger, slot_on_uiapp, event, log_msg, processed_msg);
                ...
                EventType::Slider(value) => {
                    ...
                    let processed_msg = ProcessedType::Slider {
                        message: log_msg.clone(),
                    };
                    process_event!(queue, logger, slot_on_uiapp, event, log_msg, processed_msg);
                    return;
                ...
                EventType::Combo { message: msg, _id } => {
                    ...
                    let processed_msg = ProcessedType::_Combo {
                        _id: *_id,
                        message: log_msg.clone(),
                    };
                    process_event!(queue, logger, slot_on_uiapp, event, log_msg, processed_msg);
                ...
                EventType::ApplicationCommand(cmd) => {
                    match cmd.as_str() {
                        "Clear Logger" => {
                            ...
                            let processed_msg = ProcessedType::_ApplicationCommand {
                                message: log_msg.clone(),
                            };
                            process_event!(
                                queue,
                        ...
                        "OS Info" => {
                            ...
                            process_event!(
                                ...
                                event,
                                log_msg,
                                ProcessedType::_ApplicationCommand {
                                    message: log_msg.clone()
                                }
                            );
                            return;
...
fn main() {
    ...
    let logger_text =
        Value::new("Welcome to egui_mobius ui_refresh_events example ....\n".to_string());
